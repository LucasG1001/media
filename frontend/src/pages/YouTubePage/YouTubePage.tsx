...
// Sugestão na faixa do card (a da barra de filtro é configurável).
...
export function YouTubePage() {
  ...
  const { value: suggestionLimit } = useAppSetting<number>(TAG_SUGGESTIONS_KEY, DEFAULT_TAG_SUGGESTIONS);
  ...
  // Tags filtradas combinam em E: o vídeo precisa ter todas as selecionadas.
  const visible = useMemo(
    () => searched.filter((e) => tagFilter.every((t) => e.tags.includes(t))),
    [searched, tagFilter]
  );

  // Contagem sobre o conjunto já visível = "quantos resultados se eu marcar essa
  // tag", com a interseção das selecionadas já aplicada. É daqui que sai a faixa de
  // sugestão, então toda tag sugerida tem pelo menos um resultado por construção.
  const tagCounts = useMemo(() => {
    const counts = new Map<string, number>();
    for (const e of visible) for (const tag of e.tags) counts.set(tag, (counts.get(tag) ?? 0) + 1);
    return counts;
  }, [visible]);

  // Sugestão: mais usada primeiro, desempate alfabético (sem isso a faixa trocaria
  // de ordem entre renders). Tag já selecionada não se sugere.
  const suggestions = useMemo(
    () =>
      [...tagCounts.entries()]
        ...
        .map(([tag]) => tag),
    [tagCounts, tagFilter, suggestionLimit]
  );
  ...
  const filtering = search.trim() !== "" || tagFilter.length > 0;

  const gridKey = [activeStatus, search, tagFilter.join(","), sort.field, sort.dir].join("-");
  ...
  return (
    <div className={styles.page}>
      ...
      <TagSuggestionRow
        ...
        onPick={(tag) => setTagFilter((prev) => [...prev, tag])}
      />
      ...
      <SelectedTagRow
        tags={tagFilter}
        onRemove={(tag) => setTagFilter((prev) => prev.filter((t) => t !== tag))}
